...
const UpdateStaffDocuments = ({
  ...
  currentUser,
  refetchStaff,
  refetchStaffContents,
}) => {
  const { partyType, typeId, formName, label, value } = documentInfo;
  ...
  const updateContents = (contents) => {
    updateStaffDocMutate
      ?.mutateAsync({ body: contents, pathParams: { partyId: profileId } })
      .then((res) => {
...
export default connect(mapStateToProps)(UpdateStaffDocuments);
