...
import { Input, Button, Popover, Select, Form, Row, Col, DatePicker, Space } from 'antd';
import {
  ArrowRightOutlined,
  CheckCircleOutlined,
  CloseOutlined,
...
const FilterPMS = ({
  ...
  const onFinishFilter = (values) => {
    if (form?.getFieldValue('familyId') || form?.getFieldValue('branchId')) {
      ...
      if (form?.getFieldValue('branchId')) {
        query = { ...query, isHeadQuarter: false };
      }
          ...
          <Form onFinish={onFinishFilter} form={form} colon={false} hideRequiredMark>
            ...
            <div className={currentUser?.isHeadquarter ? 'block' : 'hidden'}>
              <Form.Item name="branchId">
                ...
                >
                  {hospitalList?.records.map((family) => (
            ...
            <div className={classNames('formLabel text-left')}>Product Family</div>
  ...
  const getAllCustomers = (value) => {
    dispatch({
      type: 'customer/allCustomers',
      payload: {
        query: { isActive: true, viewSize: 1000, keyword: value },
      },
    });
  };
    ...
    <div className="w-full">
      <div className="mb-2">
        <Input
                ...
                <Popover
                  ...
                  title={
                    <div className="flex items-center justify-between">
                      <div className="text-blue-900 font-semibold py-2">Filter PMS where</div>
                      <div className="flex">
                        <div className="mx-2">
                          <Button
                            type="default"
          ...
          placeholder="Enter company name to search the PMS"
          allowClear
        />
      </div>
    </div>
...
export default connect(({ user, loading, product, customer, hospital }) => ({
  loading:
